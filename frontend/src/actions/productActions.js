  ...
  async (dispatch) => {
    try {
      ...
      const { data } = await axios.get(
        `/api/products?pageNumber=${pageNumber}&filterBy=${filterBy}`
      );
...
export const updateProduct = (values) => async (dispatch, getState) => {
  try {
    dispatch({
    ...
    const { data } = await axios.put(`/api/products/:id`, values, config);
